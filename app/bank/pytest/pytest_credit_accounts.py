...
class TestCreditAccount:
    ...
    credit_accounts_url = "/api/credit-accounts/"
    ...
    def test_create_credit_account(self, api_client):
        ...
        response = api_client.post(
            self.credit_accounts_url, account_data, format="json"
        )
    ...
    def test_get_credit_account(self, api_client, credit_account):
        response = api_client.get(f"{self.credit_accounts_url}{credit_account.id}/")
    ...
    def test_update_credit_account(self, api_client, credit_account):
        ...
        response = api_client.patch(
            f"{self.credit_accounts_url}{credit_account.id}/",
            update_data,
            format="json",
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_retrieve_credit_account(self, api_client, credit_account):
        response = api_client.get(self.credit_accounts_url)
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data.get("results")) == 1

    def test_delete_credit_account(self, api_client, credit_account):
        response = api_client.delete(f"{self.credit_accounts_url}{credit_account.id}/")
